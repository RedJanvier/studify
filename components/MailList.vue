...
<template>
  <ScrollArea class="h-[calc(100vh-65px)] flex">
    <div class="flex-1 flex flex-col gap-2 p-4 pt-0 w-full">
      <div class="flex gap-4 max-w-screen">
        <div class="">
          <h2 class="text-4xl block pt-4 md:hidden">Hello, {{ user.firstName }}</h2>
          ...
          <div class="section-header flex justify-between items-center py-4">
            ...
            <div class="section-header__actions flex gap-2">
              <button class="rounded-full bg-white p-3"><Icon icon="mdi:arrow-left" /></button>
            ...
            <div class="bg-white p-4 rounded-2xl pr-0">
              <div class="flex overflow-x-scroll snap-x hide-scroll-bar">
                <ul class="flex flex-nowrap">
                  <li class="inline-block px-3" v-for="course in courses" :key="course.id">
                    <div class="grid gap-2 w-[350px] snap-start max-w-xs overflow-hidden rounded-lg hover:shadow-xl transition-shadow duration-300 ease-in-out">
                      ...
                      <h4>{{ course.title }}</h4>
                      <Progress color="var(--main-color)" v-model="course.progress" />
                      <div :class="course.mentor.active?'flex gap-2 items-center text-indigo':'flex gap-2 items-center'">
          ...
          <div class="p-4 bg-gray-100 rounded-2xl">
            <ul class="grid gap-6">
              <li :key="item.id" v-for="item in mentors">
                <div :class="item.active?'flex gap-2 items-center text-indigo':'flex gap-2 items-center'">
                  <div class="user-img rounded-full overflow-hidden">
